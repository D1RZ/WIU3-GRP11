...
public class Firefly : Enemy
{
    ...
    private Vector2 strafeStartPos;
    ...
    void Strafe()
    {
        Vector2 PlayerEnemyVec = (PlayerPivot.transform.position - transform.position).normalized;
        ...
        if (MoveRight)
        {
            if (!isStrafing)
            {
                strafeStartPos = transform.position;
                isStrafing = true;
            }
            transform.position += (Vector3)PerpendicularRightVector(PlayerEnemyVec) * strafeSpeed * Time.deltaTime;

            if (Vector2.Distance(strafeStartPos, transform.position) > strafeDistance)
            {
                MoveRight = !MoveRight;
                isStrafing = false;
            }
        ...
        else
        {
            ...
            transform.position += (Vector3)PerpendicularLeftVector(PlayerEnemyVec) * strafeSpeed * Time.deltaTime;
